...
const agent = new Agent(new Player(1));
const human = new Human(new Player(2));
const env = new GameEnv(3);

env.boardSetMove(0, 0, agent.player.token);
env.boardSetMove(2, 2, agent.player.token);
env.boardSetMove(2, 0, agent.player.token);

env.boardSetMove(1, 1, human.player.token);
env.boardSetMove(0, 2, human.player.token);
//env.boardSetMove(2, 1, human.player.token);

console.log("Test-0");
env.boardConfig();

tree = new GameTree(env,[0,10,-10]);

//let best_move = tree.minMax(0,human.player.token,this.agent.player.token,true);

//console.log(best_move);

new_env = new GameEnv(3);
new_env = env.boardCopy();

console.log("Test-1:",env.boardCopy());

console.log("Test-2:");
console.log(new_env.boardConfig());