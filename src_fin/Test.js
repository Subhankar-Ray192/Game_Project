	...
	{	
		...
		for(let row=0; row<this.size; row++)
		{
			for(let col=0; col<this.size; col++)
			{
				...
				console.log("Before-Adding-Click:",this.clickF);
	...
	boardClearClickEvent(move_history,cell,token_img)
	{
		for(let row=0; row<this.size; row++)
		{
			for(let col=0; col<this.size; col++)
			{
				...
				console.log("Event-Removal:",this.clickF);
			}
		}
	}
...
class Agent
{
	...
	getData(env,avail_score,human)
	{
		...
		console.log("Tree-Moves:",best_move);
...
class GameCon
{
	...
	async executeTurns() 
	{
		...
		while (this.turn_counter <= 9) 
		{
			...
			win_counter = this.env.boardScoreEval(this.agent.player.token,this.avail_score);
			
			console.log(win_counter,this.env.boardScoreEval(this.agent.player.token,this.avail_score));
			if(win_counter == -10)
			{
		...
		if(this.env.isGameEnd() && draw_stat)
		{
	...
	stopGame() 
	{
		...
		let win_move = [];
		let ufc = new Util(this.env.board,this.agent.player.token);
		
		if(ufc.isRow(this.env.size))
		{
			win_move = ufc.isRowIndex(this.env.size);
			
			this.env.boardSetImage(win_move, 0, this.cell, this.win_token_img, this.agent.player.token);
			this.env.boardSetImage(win_move, 1, this.cell, this.win_token_img, this.agent.player.token);
			this.env.boardSetImage(win_move, 2, this.cell, this.win_token_img, this.agent.player.token);
		}
		
		if(ufc.isCol(this.env.size))
		{
			win_move = ufc.isColIndex(this.env.size);
			
			this.env.boardSetImage(0, win_move, this.cell, this.win_token_img, this.agent.player.token);
			this.env.boardSetImage(1, win_move, this.cell, this.win_token_img, this.agent.player.token);
			this.env.boardSetImage(2, win_move, this.cell, this.win_token_img, this.agent.player.token);
		}

		if(ufc.isPrimaryD(this.env.size))
		{
			win_move = ufc.isPrimaryDIndex(this.env.size);
			
			this.env.boardSetImage(win_move[0], win_move[0], this.cell, this.win_token_img, this.agent.player.token);
			this.env.boardSetImage(win_move[1], win_move[1], this.cell, this.win_token_img, this.agent.player.token);
			this.env.boardSetImage(win_move[2], win_move[2], this.cell, this.win_token_img, this.agent.player.token);
		}

		if(ufc.isSecondaryD(this.env.size))
		{
			win_move = ufc.isSecondaryDIndex(this.env.size);
			
			this.env.boardSetImage(win_move[0], this.env.size - win_move[0] -1, this.cell, this.win_token_img, this.agent.player.token);
			this.env.boardSetImage(win_move[1], this.env.size - win_move[1] -1, this.cell, this.win_token_img, this.agent.player.token);
			this.env.boardSetImage(win_move[2], this.env.size - win_move[2] -1, this.cell, this.win_token_img, this.agent.player.token);
		}
	...
	{        
		return new Promise((resolve,reject) => {
			...
			const cellClickListener = (event) => {
				let c_index = event.detail.index;

				console.log("Human-Index:" + c_index);
				...
				document.removeEventListener('cellClick', cellClickListener);
			...
			document.addEventListener('cellClick', cellClickListener);
	...
	{       
		...
		console.log("AI-MOVE:",move[1],move[2]);
		...
		console.log(this.env.boardConfig());
	}
	
	
}

const playEvent = document.getElementById('playButton');
const replayEvent = document.getElementById('replayButton');

playEvent.addEventListener('click', play);

async function play()
   {
       playEvent.removeEventListener('click', play);
       playEvent.style.backgroundColor = '#d3d3d3';
       
       replayEvent.addEventListener('click',replay);
       replayEvent.style.backgroundColor = '#fff';

       const game_con = new GameCon();
       await game_con.startGame();
   }


function replay()
{
       replayEvent.removeEventListener('click', replay);
       replayEvent.style.backgroundColor = '#d3d3d3';

       playEvent.addEventListener('click',play);  
       playEvent.style.backgroundColor = '#fff';
}
